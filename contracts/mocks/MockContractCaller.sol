 ...
 *      correctly blocks contract callers from buying entries (MEDIUM-1).
 */
contract MockContractCaller {
    function tryBuyEntries(address pool) external payable {
        ...
        goldPositions[0] = 0;
        IHypePoolBuyEntries(pool).buyEntries{value: msg.value}(
            whites,
            goldNums,
            goldPositions,
            address(0)
