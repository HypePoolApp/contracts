 ...
 *      triggerPublicDraw() on HypePoolV1 (Hyperliquid L1).
 *   2. HypePoolV1._triggerDraw() sends CCIP message → HypePoolVRFRequester
 *      with payload: abi.encode(roundId).
 *   3. HypePoolVRFRequester.ccipReceive() requests VRF randomness.
 *   4. fulfillRandomWords() sends CCIP message → HypePoolV1
 *      with payload: abi.encode(roundId, randomWord).
 *   5. HypePoolV1.ccipReceive() applies the randomness and sets the
 *      round to DRAWN.
 *
 * Scheduling
 * ──────────
 * An off-chain Node.js server monitors the pool and calls
 ...
 */
contract HypePoolVRFRequester is VRFConsumerBaseV2Plus, IAny2EVMMessageReceiver {
    // ─────────────── CCIP Config ─────────────────────────────────────
    ...
    /// @notice HypePoolV1 proxy address on Hyperliquid L1.
    ...
    ///         (covers _applyRandomness + state writes in HypePoolV1).
    ...
    // ─────────────── State ───────────────────────────────────────────
    /// @notice Maps VRF requestId → pool roundId.
    ...
    /// @notice Default gas budget for HypePoolV1.ccipReceive() on the
     ...
     * @param _poolContract     HypePoolV1 proxy on Hyperliquid L1.
    ...
    constructor(
        ...
        require(_poolContract != address(0), "zero pool contract");
     ...
     *                HypePoolV1 calls _triggerDraw() which sends this.
     */
    function ccipReceive(
        ...
        require(sender == poolContract, "unknown sender");
     ...
     *         HypePoolV1 via CCIP.  Call this after topping up the
     *         contract's ETH balance when pendingRandomWords[roundId] != 0.
     * @param  roundId The pool round whose stored random word to send.
     */
    ...
    /**
     * @notice Update the HypePoolV1 proxy address.
     */
    function setPoolContract(address newContract) external onlyOwner {
        require(newContract != address(0), "zero address");
        poolContract = newContract;
        emit PoolContractUpdated(newContract);
    }
     ...
     *         on HypePoolV1 if a new VRF request will be issued for the
    ...
    function _requestVRFForRound(uint256 roundId) internal {
        ...
        // on HypePoolV1 resets the round to OPEN without notifying this contract).
        require(
