/**
 * HypePoolVRFRequester.test.js
 *
 * Integration tests for HypePoolVRFRequester using @chainlink/local's
 * CCIPLocalSimulator so that the full two-contract CCIP pipeline runs
 * synchronously in a single Hardhat network.
 *
 * Architecture under test:
 *
 *   HypePoolV1 (Hyperliquid L1 side)
 *     ─── ccipSend(roundId) ──────────────────► HypePoolVRFRequester.ccipReceive()
 *                                                 └─► MockVRFCoordinator.requestRandomWords()
 *   HypePoolVRFRequester.fulfillRandomWords()
 *     ─── ccipSend(roundId, randomWord) ──────► HypePoolV1.ccipReceive()
...
describe('HypePoolVRFRequester', function () {
    // ─── Fixture ──────────────────────────────────────────────────────────────
    ...
    async function deployFixture() {
        ...
        // ── HypePoolVRFRequester (source-chain side) ────────────────────────────
        ...
        const vrfRequester = await HypePoolVRFRequesterFactory.deploy(
            ...
            CHAIN_SELECTOR,              // destChainSelector (HypePool "chain")
        ...
        // ── HypePoolV1 UUPS proxy (destination-chain side) ────────────────
        ...
        // ── Wire: tell vrfRequester the real pool address ──────────────────
        await vrfRequester.connect(owner).setPoolContract(await pool.getAddress());

        return {
            simulator, CHAIN_SELECTOR, routerAddr,
            mockVRF, vrfRequester, pool,
            owner, alice, bob,
        };
    }

    // ─── Helpers ──────────────────────────────────────────────────────────────
    const DRAW_GRACE = 300n;    // seconds — HypePoolV1.DRAW_GRACE_PERIOD

    async function buyEntry(pool, signer) {
        const price = await pool.entryPrice();
        return pool.connect(signer).buyEntries([[1, 2, 3, 4, 5]], [10], [0], ethers.ZeroAddress, { value: price });
    }

    async function closeDraw(pool) {
        await time.increase(86400); // upkeepInterval default
        return pool.closeBettingAndDraw();
    }
    ...
    describe('Deployment', function () {
        it('initialises state variables correctly', async function () {
            const { vrfRequester, mockVRF, pool, owner, routerAddr, CHAIN_SELECTOR } =
        await loadFixture(deployFixture);
            ...
            expect(await vrfRequester.poolContract()).to.equal(await pool.getAddress());
    ...
    describe('Full CCIP pipeline (closeDraw → VRF → draw result)', function () {
        it('closeDraw triggers VRF request (pendingRoundId and latestVrfRequestId set)', async function () {
            const { vrfRequester, pool, alice } = await loadFixture(deployFixture);
            await buyEntry(pool, alice);
        ...
        it('VRF fulfillment completes the second hop and round is DRAWN', async function () {
            const { vrfRequester, mockVRF, pool, alice } = await loadFixture(deployFixture);
            await buyEntry(pool, alice);
            await closeDraw(pool);
        ...
        it('full round: buy → close → fulfill → settle works end-to-end', async function () {
            const { mockVRF, pool, alice } = await loadFixture(deployFixture);
            await buyEntry(pool, alice);
            await closeDraw(pool);
            await mockVRF.fulfillRandomWords(1n, [42n]);
            await pool.settleRound();

            expect(await pool.currentRound()).to.equal(2n);
        });

        it('two consecutive rounds work correctly (requestIds increment)', async function () {
            const { mockVRF, pool, alice, bob } = await loadFixture(deployFixture);

            // Round 1
            await buyEntry(pool, alice);
            await closeDraw(pool);                      // VRF requestId = 1
            await mockVRF.fulfillRandomWords(1n, [11n]);
            await pool.settleRound();

            // Round 2
            await buyEntry(pool, bob);
            await closeDraw(pool);                      // VRF requestId = 2
            await mockVRF.fulfillRandomWords(2n, [22n]);
            await pool.settleRound();

            expect(await pool.currentRound()).to.equal(3n);
        });
    });
    ...
    describe('ccipReceive — draw request path', function () {
        ...
        it('reverts when sender is not the registered poolContract', async function () {
            const { vrfRequester, pool, alice, routerAddr, CHAIN_SELECTOR } =
        await loadFixture(deployFixture);
            // Temporarily deploy a second requester acting as a "bad pool"
            const badMsg = {
                messageId:           ethers.ZeroHash,
                sourceChainSelector: CHAIN_SELECTOR,
                sender:              ethers.AbiCoder.defaultAbiCoder().encode(['address'], [alice.address]),
        ...
        it('reverts when a VRF request is already pending for a DIFFERENT round', async function () {
            ...
            await buyEntry(pool, alice);
            await closeDraw(pool);                 // pendingRoundId = 1, requestId = 1
            ...
            await buyEntry(pool, alice);
            await closeDraw(pool);                 // pendingRoundId = 2, requestId = 2
            ...
            // We cannot inject this via CCIP without a real second pool, so we just assert state.
    ...
    describe('Emergency cancel + re-draw', function () {
        it('allows re-request for the same round after emergencyCancelDraw', async function () {
            ...
            // Cancel draw on HypePoolV1 side
            ...
            // vrfRequester still has pendingRoundId = 1 (pool didn't notify it)
            expect(await vrfRequester.pendingRoundId()).to.equal(1n);
        ...
        it('late VRF callback after round DRAWN is rejected (unexpected state)', async function () {
            ...
            await closeDraw(pool);          // requestId=1
            ...
            await pool.settleRound();       // round 1 → SETTLED
            // Any stale VRF callback would propagate a ccipSend that gets rejected by
            // the pool's ccipReceive ("unexpected state") → ReceiverError → MockVRF "callback failed"
        });
    });
    ...
    describe('Admin setters', function () {
        it('setPoolContract: owner can update', async function () {
            const { vrfRequester, owner, alice } = await loadFixture(deployFixture);
            await expect(
                vrfRequester.connect(owner).setPoolContract(alice.address),
            ).to.emit(vrfRequester, 'PoolContractUpdated').withArgs(alice.address);
            expect(await vrfRequester.poolContract()).to.equal(alice.address);
        });

        it('setPoolContract: reverts for zero address', async function () {
            const { vrfRequester, owner } = await loadFixture(deployFixture);
            await expect(
                vrfRequester.connect(owner).setPoolContract(ethers.ZeroAddress),
            ).to.be.revertedWith('zero address');
        });

        it('setPoolContract: reverts for non-owner', async function () {
            const { vrfRequester, alice } = await loadFixture(deployFixture);
            await expect(
                vrfRequester.connect(alice).setPoolContract(alice.address),
            ).to.be.reverted;
        });
